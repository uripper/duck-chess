...
def piece_symbol(duck_type: PieceType) -> str:
    return typing.cast(str, DUCK_SYMBOLS[duck_type])

def piece_name(duck_type: PieceType) -> str:
    return typing.cast(str, DUCK_NAMES[duck_type])

def duck_symbol(duck_type: PieceType) -> str:
    try:
        return typing.cast(str, DUCK_SYMBOLS[duck_type])
    except TypeError:
        print(f"Something went wrong with Duck Symbols and {duck_type}")
def duck_name(duck_type: PieceType) -> str:
    return typing.cast(str, DUCK_NAMES[duck_type])
...
class Piece:
    ...
    def unicode_symbol(self, *, invert_color: bool = False) -> str:
        ...
        return UNICODE_DUCK_SYMBOLS[symbol]
    ...
    def from_symbol(cls, symbol: str) -> Piece:
        ...
        """
        return cls(DUCK_SYMBOLS.index(symbol.lower()), symbol.isupper())
...
class Board(BaseBoard):
    ...
    def __init__(self: BoardT, fen: Optional[str] = STARTING_FEN, *, chess960: bool = False) -> None:
        ...
        if fen is None:
            ...
            self.set_fen(fen)
    @property
    def duck_moves(self) -> DuckMoveGenerator:
        """
        A list of all legal duck moves.
        """
        return DuckMoveGenerator(self)
    ...
    def find_duck(self, color: Color) -> Optional[Square]:
        """
        Finds the duck square. Returns ``None`` if there
        is no duck.
        """
        duck_mask = self.occupied_co[color] & self.ducks
        return msb(duck_mask) if duck_mask else None
    ...
    def generate_duck_moves(self, from_mask: Bitboard = BB_ALL, to_mask: Bitboard = BB_ALL, drop_mask: Bitboard = BB_ALL) -> Iterator[Move]:
        ...
        :param from_mask: Where the duck is, if applicable.
        :param to_mask: Where the duck can be placed.
        """
        ...
        if from_square == None:
            ...
            for to_square in scan_reversed(self.occupied_co[self.duck_turn] & to_mask):
                yield Move.null() | Move.from_square(to_square) | Move.drop(piece)
    ...
    def parse_san(self, san: str) -> Move:
        ...
        if match.group(1):
            piece_type = DUCK_SYMBOLS.index(match.group(1).lower())
            from_mask &= self.pieces_mask(piece_type, self.turn)
    ...
    def copy(self: BoardT, *, stack: Union[bool, int] = True) -> BoardT:
        ...
        board.fullmove_number = self.fullmove_number
        board.halfmove_clock = self.halfmove_clock
        board.duck_turn = self.duck_turn
...
class DuckMoveGenerator:
    
    def __init__(self, board: Board) -> None:
        self.board = board

    def __bool__(self) -> bool:
        return any(self.board.generate_duck_moves())
    ...
    def __iter__(self) -> Iterator[Move]:
        return self.board.generate_duck_moves()

    # def __contains__(self, move: Move) -> bool:
    #     return self.board.is_legal_duck(move)

    def __repr__(self) -> str:
        sans = ", ".join(self.board.san(move) for move in self)
        return f"<DuckMoveGenerator at {id(self):#x} ({sans})>"
